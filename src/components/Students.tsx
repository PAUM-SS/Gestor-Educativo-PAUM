...
function getStatusBadgeClass(status: Student['status']): string {
  switch (status) {
    case 'activo':
      return 'bg-[#E8F4FD] text-gb-primary';
    case 'en_riesgo':
      return 'bg-amber-50 text-amber-700 border border-amber-100';
    case 'en_rotacion':
      return 'bg-purple-50 text-purple-600';
    case 'práctica_profesional':
...
function getGpaDisplay(student: Student): { label: string; className: string } {
  const isGpaUnknown = Boolean(student.kardex) && student.kardex?.extracted?.gpa === undefined;
  if (isGpaUnknown) {
    return { label: 'N/D', className: 'text-slate-500' };
  }
  if (student.gpa >= 9.0) return { label: student.gpa.toFixed(2), className: 'text-emerald-700' };
  if (student.gpa >= 8.0) return { label: student.gpa.toFixed(2), className: 'text-blue-700' };
  return { label: student.gpa.toFixed(2), className: 'text-amber-700' };
}
...
function getStudentProgress(student: Student): number {
  const fromKardex = student.kardex?.extracted?.progressPercent;
  if (fromKardex !== undefined) return fromKardex;
  return Math.min(100, Math.round((student.semester / 12) * 100));
}

function resolveApprovedModules(student: Student) {
  if (student.kardex?.matchedModuleIds?.length) {
    return MOCK_MODULES.filter(m => student.kardex!.matchedModuleIds.includes(m.id));
  }
  return MOCK_MODULES.filter(m => (m.semester !== 'Servicio' ? (m.semester as number) < student.semester : false));
}

function resolvePendingModules(student: Student) {
  if (student.kardex?.missingModuleIds?.length) {
    return MOCK_MODULES.filter(m => student.kardex!.missingModuleIds.includes(m.id));
  }
  return MOCK_MODULES.filter(m => (m.semester === 'Servicio' || (m.semester as number) >= student.semester));
}
...
export default function Students() {
  ...
  const [kardexDocsOpen, setKardexDocsOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && !isEditingStudent) handleCloseDetails();
    };
  ...
  const handleStartEdit = () => {
    ...
    setEditProgressPercent(getStudentProgress(selectedStudent));
  ...
  const handleCancelEdit = () => {
    ...
    setEditStudentDraft({});
    setEditProgressPercent(0);
  };

  const handleSaveStudentEdit = async () => {
    if (!selectedStudent) return;
    const payload: Partial<Student> = { ...editStudentDraft };
    if (selectedStudent.kardex) {
      payload.kardex = {
        ...selectedStudent.kardex,
        extracted: {
          ...selectedStudent.kardex.extracted,
          progressPercent: editProgressPercent,
        },
      };
    }
    const updated = await executeUpdate(
      () => studentService.updateStudent(selectedStudent.id, payload),
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {selectedStudent && (() => {
        const progress = isEditingStudent ? editProgressPercent : getStudentProgress(selectedStudent);
        const gpaDisplay = getGpaDisplay(isEditingStudent ? { ...selectedStudent, ...editStudentDraft } as Student : selectedStudent);
        const approvedModules = resolveApprovedModules(selectedStudent);
        const pendingModules = resolvePendingModules(selectedStudent);
        const studentTypeLabel = selectedStudent.kardex?.extracted?.studentStatusLabel;
        const hasKardexDocs = Boolean(
        ...
        const currentEnrollments: { nrc: string; title: string; faculty: string; schedule: string; room: string }[] = [];
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-display font-bold text-slate-800">Expediente Académico</h2>
                <p className="text-[11px] text-slate-400 font-medium mt-0.5 uppercase tracking-widest">
                  {isEditingStudent ? 'Modo edición' : 'Ficha del alumno'}
                </p>
              </div>
              <Button
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="flex gap-5 items-start">
                <div className="w-[4.5rem] h-[4.5rem] rounded-2xl bg-slate-100 flex items-center justify-center text-slate-700 font-bold text-lg shrink-0 border border-slate-200 tracking-tight">
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  <div className="flex items-start gap-3 flex-wrap">
                    {isEditingStudent ? (
                      <input
                        ...
                        className="flex-1 min-w-0 text-2xl font-bold text-slate-900 bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 focus:ring-2 focus:ring-gb-primary/20 focus:border-gb-primary outline-none"
                      />
                    ) : (
                      <h3 className="text-3xl font-bold text-slate-900 leading-tight">{selectedStudent.name}</h3>
                    )}
                    <span className={`shrink-0 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border ${
                  ...
                  {/* Matrícula y correo (secundarios) */}
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-500">
                    {isEditingStudent ? (
                      <>
                        <input
                          value={editStudentDraft.enrollmentId ?? ''}
                          onChange={e => setEditStudentDraft(d => ({ ...d, enrollmentId: e.target.value }))}
                          className="font-mono text-sm bg-slate-50 border border-slate-200 rounded-lg px-2 py-0.5 focus:ring-2 focus:ring-gb-primary/20 outline-none w-36"
                        ...
                        <input
                          ...
                          className="text-sm bg-slate-50 border border-slate-200 rounded-lg px-2 py-0.5 focus:ring-2 focus:ring-gb-primary/20 outline-none"
                          placeholder="Correo"
                        />
                      </>
                    ) : (
                      <>
                        <span className="font-mono">{selectedStudent.enrollmentId}</span>
                  ...
                  {/* Metadatos: cohorte, semestre, tipo */}
                  <div className="flex flex-wrap gap-2 pt-0.5">
                    {isEditingStudent ? (
                      <>
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-black text-slate-400 uppercase">Estatus</span>
                          <select
                            value={editStudentDraft.status ?? 'activo'}
                            onChange={e => setEditStudentDraft(d => ({ ...d, status: e.target.value as Student['status'] }))}
                            className="text-xs bg-slate-50 border border-slate-200 rounded-lg px-2 py-0.5 focus:ring-2 focus:ring-gb-primary/20 outline-none"
                          >
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-black text-slate-400 uppercase">Tutor</span>
                          <input
                            value={editStudentDraft.tutor ?? ''}
                            onChange={e => setEditStudentDraft(d => ({ ...d, tutor: e.target.value }))}
                            className="w-40 text-xs bg-slate-50 border border-slate-200 rounded-lg px-2 py-0.5 focus:ring-2 focus:ring-gb-primary/20 outline-none"
                          />
                        </div>
                      </>
                    ) : (
                      <>
                        <span className="px-2 py-0.5 rounded-md text-xs text-slate-600 border border-slate-200">
              ...
              {selectedStudent.alert && selectedStudent.kardex?.riskReasons?.length ? (
                <div className="rounded-xl border border-red-200 bg-red-50/60 p-3 text-xs text-red-800">
                  <div className="font-bold mb-1.5 flex items-center gap-2 text-red-700">
              ...
              <div>
                <h4 className="text-[12px] font-black uppercase text-slate-400 tracking-widest mb-3">
                  Indicadores académicos
                </h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {/* Avance */}
                  <div className="rounded-xl border border-slate-200 p-3 bg-white">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">Avance</p>
                    {isEditingStudent ? (
                      <input
                        type="number" min="0" max="100"
                        value={editProgressPercent}
                        onChange={e => setEditProgressPercent(Number(e.target.value))}
                        className="w-full h-9 px-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold focus:ring-2 focus:ring-gb-primary/20 outline-none"
                      />
                    ) : (
                      <>
                  ...
                  {/* GPA */}
                  <div className="rounded-xl border border-slate-200 p-3 bg-white">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">GPA</p>
                    {isEditingStudent ? (
                      <input
                      ...
                      <p className={`text-xl font-bold ${gpaDisplay.className}`}>{gpaDisplay.label}</p>
                    )}
                  </div>
                  {/* Asistencia */}
                  <div className="rounded-xl border border-slate-200 p-3 bg-white">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">Asistencia</p>
                    {isEditingStudent ? (
                      <input
                      ...
                      <p className={`text-xl font-bold ${selectedStudent.attendance < 80 ? 'text-red-700' : 'text-slate-900'}`}>
                  ...
                  {/* Semestre */}
                  <div className="rounded-xl border border-slate-200 p-3 bg-white">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">Semestre</p>
                    {isEditingStudent ? (
                      <input
                      ...
                      <p className="text-xl font-bold text-slate-900">{selectedStudent.semester}º</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                {currentEnrollments.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-6 text-center">
                    ...
                    <p className="text-sm font-medium text-slate-600">Sin inscripciones activas</p>
                    <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                      Las materias inscritas aparecerán aquí con NRC, docente, horario y salón cuando se vincule el calendario académico.
                    </p>
                  </div>
                ) : (
                  <div className="grid gap-3">
                    {currentEnrollments.map(course => (
                      <div key={course.nrc} className="rounded-xl border border-slate-200 p-4 bg-white">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="font-semibold text-slate-800">{course.title}</p>
                            <p className="text-xs text-slate-500 mt-0.5">NRC {course.nrc} · {course.faculty}</p>
                          </div>
                        </div>
                        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                          <span>{course.schedule}</span>
                          <span>Salón {course.room}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* ── 5. Materias aprobadas y pendientes ── */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-slate-500 tabular-nums">{approvedModules.length}</span>
                  </div>
                  <div className="p-4 max-h-48 overflow-y-auto">
                    {approvedModules.length === 0 ? (
                      <p className="text-slate-400 italic text-xs">Sin materias detectadas en el kardex.</p>
                    ) : (
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
                    <h4 className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                      <BookX size={16} className="text-amber-600" />
                      Materias pendientes
                    </h4>
                    <span className="text-xs font-bold text-slate-500 tabular-nums">{pendingModules.length}</span>
                  </div>
                  <div className="p-4 max-h-48 overflow-y-auto">
                    {pendingModules.length === 0 ? (
                      <p className="text-slate-400 italic text-xs">No hay materias pendientes registradas.</p>
                    ) : (
                      <ul className="space-y-1.5">
              ...
              {/* ── 6. Kardex / documentación (colapsable) ── */}
              {hasKardexDocs && (
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  <button
                    type="button"
                    onClick={() => setKardexDocsOpen(o => !o)}
                    className="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-slate-50 transition-colors"
                  >
                    <span className="font-bold text-slate-800 flex items-center gap-2 text-sm">
                      <FileText size={16} className="text-slate-500" />
                      Kardex y documentación
                    </span>
                    <ChevronDown
                      size={18}
                      className={`text-slate-400 transition-transform ${kardexDocsOpen ? 'rotate-180' : ''}`}
                    />
                  </button>
                  {kardexDocsOpen && (
                    <div className="px-4 pb-4 pt-0 border-t border-slate-100 space-y-3">
                      {selectedStudent.kardex?.parsedAt && (
                        <p className="text-[11px] text-slate-400 pt-3">
                          Procesado: {new Date(selectedStudent.kardex.parsedAt).toLocaleString()}
                          {selectedStudent.kardex.sourceFileName && (
                            <span className="ml-1">· {selectedStudent.kardex.sourceFileName}</span>
                          )}
                        </p>
                      )}
                      <div className="flex flex-wrap gap-2">
                        {selectedStudent.kardex?.sourcePdfUrl && (
                          <a href={selectedStudent.kardex.sourcePdfUrl} target="_blank" rel="noreferrer"
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold text-slate-600 border border-slate-200 hover:border-gb-primary/30 hover:text-gb-primary transition-colors">
                        ...
                        {selectedStudent.kardex?.sourceTextUrl && (
                          <a href={selectedStudent.kardex.sourceTextUrl} target="_blank" rel="noreferrer"
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold text-slate-600 border border-slate-200 hover:border-gb-primary/30 hover:text-gb-primary transition-colors">
                            <FileText size={13} /> Texto extraído ({selectedStudent.kardex.extractedTextLength})
                          </a>
                        )}
                      </div>
                    </div>
