...
export default function Faculty() {
  ...
  useEffect(() => {
    facultyService.getFaculty().then(setFaculty);
  }, []);
  ...
  const stats = useMemo(() => {
    const fullTimeCount = faculty.filter((member) => member.dedication === 'Tiempo Completo').length;
    const definitiveCount = faculty.filter((member) => member.category !== 'Profesor de Asignatura').length;
    const avgAnnualEvaluation = faculty.length
      ? (faculty.reduce((acc, member) => acc + member.compliance.annualEvaluation, 0) / faculty.length).toFixed(1)
      : '0.0';
  ...
  const handleImportFaculty = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validFile) {
      window.alert('La importación de docentes admite archivos .json o .csv.');
      return;
    }
    ...
    try {
      const result = await facultyService.importFaculty(file);

      if (!result) {
        window.alert('No se pudo importar la base de docentes.');
        return;
      }

      setFaculty(result.faculty);
      if (selectedFaculty) {
        const refreshedFaculty = result.faculty.find((member) => member.id === selectedFaculty.id) || null;
        setSelectedFaculty(refreshedFaculty);
        setEditedProfile(refreshedFaculty ? buildEditableFacultyForm(refreshedFaculty) : null);
      }

      window.alert(`Importación completada. ${result.created} docentes creados y ${result.updated} actualizados.`);
    } finally {
  ...
  const handleRegister = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (faculty.some((member) => member.id === trimmedId)) {
      window.alert('Ya existe un academico registrado con ese ID institucional.');
      return;
    }
    ...
    try {
      const created = await facultyService.addFaculty(facultyToCreate);
      if (!created) {
        window.alert('No se pudo registrar el academico. Revise si el ID ya existe o si el servidor esta activo.');
        return;
      }
      ...
      console.error(error);
      window.alert('Ocurrio un error al registrar al academico.');
    } finally {
    ...
    <div className="space-y-8">
      <header className="flex justify-between items-end gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-bold text-sm border transition-all active:scale-95 ${
              isImporting
      ...
      <div className="geometric-card overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {faculty.map((member) => (
            <div key={member.id} className="table-row-gb grid grid-cols-[1.5fr_1fr_1fr_1fr_120px] items-center py-4">
              <div 
              ...
              >
                ...
                <div>
                  <p className="font-bold text-gb-secondary group-hover:text-gb-primary transition-colors underline-offset-4 group-hover:underline decoration-gb-primary/30">{member.name}</p>
                  <p className="text-[10px] text-gb-accent font-bold uppercase tracking-tighter">{member.category} - {member.level}</p>
                </div>
              </div>
              ...
              <div className="w-full md:w-80 bg-slate-50 border-r border-slate-100 p-8 flex flex-col items-center text-center">
                ...
                <div className="mt-auto pt-8 flex gap-2 w-full">
                  {isEditingProfile ? (
                    <button 
                      onClick={async () => {
                        ...
                        const updated = await facultyService.updateFaculty(selectedFaculty.id, {
                        ...
                        if (updated) {
                          ...
                          window.alert('No se pudo actualizar el expediente docente.');
                        }
                        setIsSavingProfile(false);
                      }}
                      disabled={isSavingProfile}
                      className={`flex-1 py-2 rounded-lg font-bold text-xs uppercase tracking-wider transition-colors ${
                        isSavingProfile ? 'bg-slate-200 text-slate-500' : 'bg-emerald-500 text-white hover:bg-emerald-600'
                      }`}
                    >
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const confirmed = window.confirm(`¿Desea dar de baja a ${selectedFaculty.name}? Las secciones quedarán como SIN ASIGNAR.`);
                      if (!confirmed) return;

                      setIsDeletingFaculty(true);
                      const success = await facultyService.deleteFaculty(selectedFaculty.id);

                      if (success) {
                        setFaculty((prev) => prev.filter((member) => member.id !== selectedFaculty.id));
                    ...
                    className={`p-2 border border-slate-200 rounded-lg transition-colors ${
                      isDeletingFaculty ? 'text-slate-300' : 'text-slate-400 hover:text-rose-500'
                    }`}
                  >
                    <Trash2 size={16} />
                  </button>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {[
                            ...
                            <button
                              key={item.key}
                              type="button"
                              onClick={() => updateEditedProfile({ [item.key]: !editedProfile[item.key as keyof EditableFacultyForm] } as Partial<EditableFacultyForm>)}
                              className={`px-3 py-2 rounded-lg text-[10px] font-bold border transition-all ${
                                editedProfile[item.key as keyof EditableFacultyForm]
                                ...
                                onClick={() =>
                                  updateEditedProfile({
                                    weeklySchedule: isActive
                                      ? editedProfile.weeklySchedule.filter((item) => item !== day)
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                  <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                    ...
                    <p className="text-lg font-bold text-gb-secondary tracking-tight">
                      {selectedFaculty.weeklySchedule?.includes(['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'][new Date().getDay()]) 
                        ? 'Presente en Facultad' 
                        : 'No programado'}
                    </p>
                      ...
                      selectedFaculty.permissions.map((perm, i) => (
                        <div key={i} className="p-4 rounded-xl border border-slate-100 flex items-start gap-4">
                          <div className={`p-2 rounded-lg ${
                            perm.type === 'Médico' ? 'bg-rose-100 text-rose-600' : 
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-500 font-medium">{perm.type} • {perm.startDate} al {perm.endDate}</p>
                          </div>
                        </div>
                      ))
                    ) : (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select 
                        ...
                        onChange={e => setNewFaculty({...newFaculty, category: e.target.value})}
                      >
                        <option>Profesor de Asignatura</option>
                        <option>Profesor-Investigador</option>
                        <option>Técnico Académico</option>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {['Tiempo Completo', 'Medio Tiempo', 'Hora Clase'].map(d => (
                        <button
                          key={d}
                          type="button"
                          onClick={() => setNewFaculty({...newFaculty, dedication: d as any})}
                          className={`py-2 rounded-lg text-[10px] font-bold border transition-all ${
                            newFaculty.dedication === d 
                              ? 'bg-gb-primary/10 border-gb-primary text-gb-primary' 
